...
def ass_header(style: SubtitleStyle) -> str:
    """ASS V4+ header (Script Info + Styles + Events format rows) for a style.

    Shared by the full document serializer and the render-time ASS built from
    the edited cue table (which may contain zero cues after the user deletes
    every subtitle).
    """
...
def ass_to_ass_text(document: Subtitle) -> str:
    ...
    return ass_header(document.style) + "\n" + "\n".join(events) + "\n"
